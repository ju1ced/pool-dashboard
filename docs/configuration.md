...
| `comfort_score`              | entity  | no       | Display-only comfort score (0-100, `input_number.*`/`sensor.*`), shown both as a badge on the pool illustration (top-right, below "Buiten") and as a row in **Instellingen**. Reads whatever value an existing automation already computes — the card never derives or synthesizes it. |
